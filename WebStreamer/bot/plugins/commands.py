...
# ─────────────────────────────────────────────
#  Bot command handlers + link generation
# ─────────────────────────────────────────────
...
async def getlink_handler(client: Client, message: Message):
    ...
    # Validate: sab numbers hone chahiye
    valid_ids = []
    for arg in args:
        try:
...
async def bulklink_handler(client: Client, message: Message):
    args = message.text.strip().split()[1:]
...
# ─────────────────────────────────────────────
@Client.on_message(filters.private & SUPPORTED_MEDIA)
...
async def _generate_links_for_ids(
    ...
    """
    Msg ID list se links generate karo.
    Returns list of result strings.
    """
    results = []
    BATCH_SIZE = 50  # Telegram ek baar mein max 50 messages deta hai
    ...
    for i in range(0, len(msg_ids), BATCH_SIZE):
        ...
        # Agar single message return hua to list mein wrap karo
...
async def _send_results(
    ...
    """
    Results ko proper chunks mein split karke bhejo.
    Telegram message limit 4096 chars hai.
    """
    if not results:
        await processing_msg.edit_text(
...
def _split_into_chunks(results: list, header: str = "", limit: int = 4000) -> list:
    """
    Results list ko Telegram-safe chunks mein split karo.
    """
    chunks = []
    current = header
