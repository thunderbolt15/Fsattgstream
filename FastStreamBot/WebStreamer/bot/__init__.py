...
# ─────────────────────────────────────────────
#  Bot client + multi-bot worker initialization
# ─────────────────────────────────────────────
...
class MultiClientManager:
    """
    Round-robin load balancer for multiple bot clients.
    Each user request gets served by a different bot → parallel fast downloads.
    """
    def __init__(self):
    ...
    async def get(self) -> Client:
        if not self._clients:
            return StreamBot  # fallback to main bot
...
async def initialize_clients():
    ...
    # Add main bot to multi-client pool
    ...
    for i, token in enumerate(Var.MULTI_TOKENS, start=1):
        try:
            worker = Client(
                ...
                bot_token=token,
                sleep_threshold=10,
                no_updates=True,  # workers sirf download karte hain
            )
            await worker.start()
            worker_me = await worker.get_me()

            # Worker ko BIN_CHANNEL mein add karo (zaruri hai)
            try:
                await StreamBot.get_chat_member(Var.BIN_CHANNEL, worker_me.id)
            except Exception:
                logger.warning(
                    f"Worker @{worker_me.username} is not in BIN_CHANNEL. "
                    f"Please add it manually as admin."
                )
...
async def stop_clients():
    """Stop all bot clients gracefully."""
